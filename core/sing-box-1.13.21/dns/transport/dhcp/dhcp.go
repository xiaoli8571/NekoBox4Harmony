...
func (t *Transport) Start(stage adapter.StartStage) error {
	...
	if t.interfaceName == "" {
		// OpenHarmony patch: interface monitor may be nil (no netlink in sandbox)
		if monitor := t.networkManager.InterfaceMonitor(); monitor != nil {
			t.interfaceCallback = monitor.RegisterCallback(t.interfaceUpdated)
		}
	}
	go func() {
		_, err := t.fetch()
