...
> 当前基线:1.5.7(versionCode 1001711)。F1 已随本基线大部分交付:`resources/dark/element/color.json` 深色资源 + EntryAbility 启动时应用 colorMode,勿重做;若验收发现仍有页面切主题需重进,按 F1 补齐。其余为本阶段要补齐的降级项。
...
- 现状:设置里切系统/浅色/深色后,部分页面要重进才刷新
- 方案:`ApplicationContext.setColorMode()`(公开 API)设置全局色彩模式;外观状态入 AppStorage,页面用 `@StorageProp` 绑定或监听变更即时刷新;自绘固定色替换为主题资源色
- 改动:`entryability/EntryAbility.ets`、`pages/SettingsPage.ets`、各页面按需
...
## F3 订阅后台定时更新(需要改 module.json5,仅限本项)
...
## F4 per-app 应用列表(需要改 module.json5,仅限本项)
